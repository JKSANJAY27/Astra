...
import { initializeParser, indexWorkspace, getCachedGraph, extractModelFromCode, extractPromptFromCode, loadCache } from './parser';
import { calculate_cost, estimate_tokens, calculate_carbon } from './cost_calculator';
...
export function activate(context: vscode.ExtensionContext) {
  ...
  if (workspaceFolders && workspaceFolders.length > 0) {
    ...
    // Fallback: Use directory of the active text editor if available
    if (vscode.window.activeTextEditor && vscode.window.activeTextEditor.document.uri.scheme === 'file') {
      workspaceRoot = require('path').dirname(vscode.window.activeTextEditor.document.uri.fsPath);
      vscode.window.showInformationMessage(`Astra: Running in single-file mode (${workspaceRoot})`);
    } else {
      // Fallback: Use temp directory if no file is open (though activation usually requires a file open event for some activationEvents)
      // or just inform usage.
      vscode.window.showWarningMessage('Astra: No workspace or file open. Please open a folder or file to start tracking.');
      return;
    }
  }
  ...
  const updateTreeviewWithAllCalls = async () => {
    ...
    for (const unit of graph.units) {
      ...
      if (classification && classification.role === 'consumer' && classification.category === 'llm') {
        ...
        const carbon = calculate_carbon(model, tokens);
    ...
    const totalCarbon = allCalls.reduce((sum, call) => sum + (call.estimated_carbon || 0), 0);
    const userCount = tree_provider.get_user_count();
    updateStatusBar(totalCarbon, userCount);
  ...
  const updateStatusBar = (totalCarbon: number, userCount: number) => {
    const config = vscode.workspace.getConfiguration('astra'); // Updated config section
    const monthlyCarbonBudget = config.get<number>('monthlyCarbonBudget') || 10000; // 10kg CO2e default

    // Calculate projected monthly footprint based on current simulation settings
    const dailyCarbon = totalCarbon * userCount;
    const monthlyCarbon = dailyCarbon * 30; // gCO2e

    // Display in kg if > 1000g
    const displayCarbon = monthlyCarbon > 1000
      ? `${(monthlyCarbon / 1000).toFixed(2)} kg`
      : `${monthlyCarbon.toFixed(2)} g`;

    const displayBudget = monthlyCarbonBudget > 1000
      ? `${(monthlyCarbonBudget / 1000).toFixed(2)} kg`
      : `${monthlyCarbonBudget} g`;

    statusBarItem.text = `$(leaf) ${displayCarbon} / ${displayBudget} CO2e`;
    statusBarItem.tooltip = `Projected Monthly Footprint: ${displayCarbon}\nCarbon Budget: ${displayBudget}`;

    // Color logic
    if (monthlyCarbon >= monthlyCarbonBudget) {
      statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.errorBackground');
    } else if (monthlyCarbon >= monthlyCarbonBudget * 0.8) {
    ...
    if (monthlyCarbon >= monthlyCarbonBudget * 0.8) {
      vscode.window.showWarningMessage(`Climate Alert: You've reached over 80% of your Carbon Budget!`);
    }
  };
  ...
  initializeParser(workspaceRoot).then(async () => {
    console.log('Parser system initialized');

    // --- 1. Fast Load from Cache (Fix for "Zeroing" issue) ---
    try {
      console.log('Attempting to load cached graph...');
      const cached = await loadCache(workspaceRoot);
      if (cached) {
        console.log(`Loaded cached graph with ${cached.units.length} units`);
        await updateTreeviewWithAllCalls();
        codelens_provider.refresh();
        tree_provider.refresh();
      }
    } catch (e) {
      console.warn('Failed to load cache on startup:', e);
    }
    ...
    vscode.window.withProgress({
      location: vscode.ProgressLocation.Window, // Move to Window (Status Bar) to be less intrusive
